...
.z-column-btn {
	...
	right: 0;
	width: 23px;
	background: url(${c:encodeURL('~./zul/img/sapphire/grid/hd-btn.png')}) no-repeat scroll left 0 transparent;
	background-position: 0 0;
}
...
.z-column-sort-asc .z-column-sort-img,
.z-column-sort-dsc .z-column-sort-img {
	background-image: url(${c:encodeURL('~./zul/img/sapphire/grid/arrows.gif')});
}
</c:if>
<c:if test="${c:browser('ie6-') or c:browser('ie7-')}">
