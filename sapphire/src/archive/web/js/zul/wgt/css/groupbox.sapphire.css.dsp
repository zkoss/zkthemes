...
.z-fieldset .z-fieldset-hl {
	...
	background-image:url(${c:encodeURL('~./zul/img/sapphire/layout/groupbox-y.gif')});
}
...
.z-fieldset-hm {
	background: no-repeat;
	background-position: 0 center;
	background-image:url(${c:encodeURL('~./zul/img/sapphire/layout/groupbox-x.gif')});
}
...
.z-groupbox-hm {
	background-image:url(${c:encodeURL('~./zul/img/sapphire/layout/borderlayout-hm.gif')});
}
</c:if>