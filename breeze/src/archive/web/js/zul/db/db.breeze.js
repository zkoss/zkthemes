...
zk.afterLoad('zul.db', function(){
	...
	/** Datebox **/
	zul.db.Datebox.prototype.bind_ = function (){
		this.$supers(zul.db.Datebox, 'bind_', arguments);
	...
	/** Calendar **/
	zul.db.Calendar.molds['default'] = function (out) {
		var uuid = this.uuid,
			view = this._view,
			zcls = this.getZclass(),
			val = this.getTime(),
		...
		case "day" :
			out.push('<tr><td colspan="3"><table id="', uuid, '-mid" class="', zcls, '-calday" width="100%" border="0" cellspacing="0" cellpadding="0">',
				...
				for (var j = 0 ; j < 7; ++j) {
					...
					else out.push(' class="z-weekday"');	//ADDED
